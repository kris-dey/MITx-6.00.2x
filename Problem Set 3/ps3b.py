...
class SimpleVirus(object):
    ...
    def reproduce(self, popDensity):
        ...
        propability = random.random()
        maxBirthProb2 = self.maxBirthProb * (1 - popDensity)  
        if propability <= maxBirthProb2:
...
class Patient(object):
    ...
    def update(self):
        ...
        viruses3 = self.viruses[:]        
        popDen = len(self.viruses)/self.maxPop
        for virus in viruses3:
            try:
                virus.reproduce(popDen)
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for i in range(numTrials):
        ...
        data = data + totalViruses
    
    means= data/numTrials
    
    pylab.plot(list(means), label = "SimpleVirus")
        ...
        if propability <= maxBirthProb2:
            ...
            for resistant in self.resistances:
               if random.random() < 1 - self.mutProb:
                   newResistances[resistant] = self.resistances[resistant]
               else:
                   newResistances[resistant] = not(self.resistances[resistant])
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        for virus in viruses3:
            try:
                virus.reproduce(popDen, self.activeDrugs)
                self.viruses.append(virus)
            except NoChildException:
                continue
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for i in range(numTrials):
        ...
        for j in range(150):
            patient.update()
            totalViruses.append(patient.getTotalPop())
        patient.activeDrugs.append('guttagonol') 
        for j in range(150):
            patient.update()
            totalViruses.append(patient.getTotalPop())         
        data = data + totalViruses
